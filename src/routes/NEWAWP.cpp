...
void RedLocalSAWP() {
    PIDDataSet TestPara={1.5,0.1,0.15};
    TurnMaxTimePID(TestPara, -52, 0.4, true);
    MoveEncoderPID(TestPara, 100, 17, 0.3, -52, true);
    RunLift(60);
    wait(1000,msec);
    ...
    MoveEncoderPID(TestPara, -70, 12, 0.3, 0, false);// slow down so Mogo doesn't get sent flying
    ...
    TurnMaxTimePID(TestPara,23, 0.6, true);//turn to face the corner for bottom ring
    MoveEncoderPID(TestPara, 60, 50, 0.3, 23, true);// drive to grab the bottom ring
    MoveEncoderPID(TestPara, 80, 15, 0.3, 23, true);
    wait(100,msec);//Wait to make sure you actually grab the ring

    MoveEncoderPID(TestPara, -60,13.6, 0.3, 23, true);// back up out of corner
    TurnMaxTimePID(TestPara, -90, 0.7, true);// Turn to face last ring in front of alliance
    MoveEncoderPID(TestPara, 80, 43, 0.3, -90, false);//drive towards last ring fast to sacve time
    
    Pistake.set(true);
    
    MoveEncoderPID(TestPara, 70, 22, 0.3, -90, true);// drive slower to last ring to not hit ring
    Pistake.set(false);
    wait(100,msec);
    MoveEncoderPID(TestPara, -100, 27, 0.3, -90, true);
    TurnMaxTimePID(TestPara, -143, 0.6, true);
    MoveEncoderPID(TestPara, 100, 35, 0.3, -143, true);
    RunLift(70);
    wait(400,msec);
    RunLift(0);
...
void BlueLocalSAWP() {
    ...
    liftSensor.resetPosition();
    MoveEncoderPID(TestPara, -100, 13 , 0.4, 0, false);//drive to Mogo Fast to save time
    MoveEncoderPID(TestPara, -70, 12.5, 0.3, 0, false);// slow down so Mogo doesn't get sent flying
   
    Clamp.set(true);
    wait(200,msec);
    RunRoller(100);
    TurnMaxTimePID(TestPara, -90, 0.5, true);// Turn to face first ring
    MoveEncoderPID(TestPara, 90, 33, 0.3, -90, true);//drive to grab first ring
    TurnMaxTimePID(TestPara,-25, 0.6, true);//turn to face the corner for bottom ring
    MoveEncoderPID(TestPara, 70, 50, 0.3, -25, false);// drive to grab the bottom ring
    MoveEncoderPID(TestPara, 90, 15, 0.3, -25, false);
    wait(100,msec);//Wait to make sure you actually grab the ring

    MoveEncoderPID(TestPara, -60,15, 0.3, -25, true);// back up out of corner doinker.set(true);
    MoveEncoderPID(TestPara, 60, 12, 0.3, -25, true);// back up out of corner
    ...
    /*
    TurnMaxTimePID(TestPara, 90 , 0.7, true);// Turn to face last ring in front of alliance
    MoveEncoderPID(TestPara, 80, 48, 0.3, 90, false);//drive towards last ring fast to sacve time
    
    Pistake.set(true);
    MoveEncoderPID(TestPara, 70, 25, 0.3, 90, true);// drive slower to last ring to not hit ring
      
    Pistake.set(false);
    wait(300,msec);
    MoveEncoderPID(TestPara, -100, 30, 0.3, 90, true);// drive slower to last ring to not hit ring
    
    
    
    
    TurnMaxTimePID(TestPara, 143, 0.6, true);
    MoveEncoderPID(TestPara, 100, 32, 0.3, 143, true);
    RunLift(60);
    wait(400,msec);
    RunLift(0);
    */
